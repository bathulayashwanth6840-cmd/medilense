...
export async function GET(
  ...
  try {
    ...
    const history = await store.getProvenanceHistory(id);
    if (!history || history.length === 0) {
      ...
      // Generate fallback virtual provenance record
      const fallbackProv = {
        ...
        sourceText: entity.sourceOriginalSnippet || 'Extracted Clinical Record',
