...
export default function SideBySideViewer({
  ...
  const docName = associatedDoc?.originalFileName || (selectedEntity ? 'Source Clinical Record' : 'No Document Attached');
  const confidenceScore = selectedEntity?.confidenceScore 
    ? (selectedEntity.confidenceScore * 100).toFixed(1) 
    : (associatedDoc ? '95.0' : '0.0');

  const handleStartEdit = () => {
    setIsEditing(true);
    setEditValue(
      selectedEntity?.measuredValue 
        ? `${selectedEntity.measuredValue} ${selectedEntity.unit || ''}`.trim() 
        : ''
    );
    setEditReason('');
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm flex flex-col overflow-hidden">
      {/* Header: Document Source / AI Inspector */}
      ...
      {/* AI Inspector / Provenance Information Panel */}
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-mono">
            {selectedEntity?.provenanceSource || (associatedDoc ? 'DOCUMENT_EXTRACTED' : 'USER_PROVIDED')}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-slate-600 dark:text-slate-400">
          ...
          <div>
            <span className="text-slate-400 text-[10px] block">Extracted Date</span>
            <span className="font-semibold text-slate-800 dark:text-slate-200">
              {associatedDoc?.uploadedAt ? formatDate(associatedDoc.uploadedAt) : 'Not available'}
            </span>
          </div>
          <div>
            <div className="flex items-center gap-1">
              ...
              <div className="space-y-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs animate-in fade-in duration-150">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Corrected value per clinician review"
                  />
                </div>
